...
function defaultOutputRoot2(framework) {
  switch (framework) {
    case "nextjs":
      return "app";
    case "astro":
      return import_node_path4.default.join("src", "pages");
    case "nuxt":
      return "pages";
    case "sveltekit":
      return import_node_path4.default.join("src", "routes");
    case "remix":
      return import_node_path4.default.join("app", "routes");
  }
}
...
async function detectFramework() {
  const packageJson = await readJsonIfExists(import_node_path4.default.join(process.cwd(), "package.json"));
  const dependencies = {
...
async function readConfig() {
  const config = await readJsonIfExists(import_node_path4.default.join(process.cwd(), "sophon.config.json"));
  return config;
}
...
async function initCommand(values) {
  ...
  const config = {
    framework,
    entitiesPath: import_node_path4.default.join("data", "entities.json"),
    pagesOutput: defaultOutputRoot2(framework),
    technicalOutput: "public",
    enrichOutput: import_node_path4.default.join("data", "enriched")
  };
  await writeGeneratedFile(
    import_node_path4.default.join(process.cwd(), "sophon.config.json"),
...
async function discoverCommand(values) {
  ...
  const outputPath = asString(values["discover-output"]) ?? asString(values.output) ?? import_node_path4.default.join("data", "entities.json");
...
async function generateCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path4.default.join("data", "entities.json");
...
async function runCommand(values) {
  const config = await readConfig();
  const framework = await resolveFramework(asString(values.framework));
  const discoverOutput = asString(values["discover-output"]) ?? asString(values.output) ?? config?.entitiesPath ?? import_node_path4.default.join("data", "entities.json");
  ...
  const enrichOutput = asString(values["enrich-output"]) ?? config?.enrichOutput ?? import_node_path4.default.join("data", "enriched");
