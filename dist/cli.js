...
var COMMERCIAL_SECTIONS, COMPARISON_SECTIONS, SEGMENTED_SECTIONS, INFORMATIONAL_SECTIONS, SECTIONS_BY_INTENT;
...
async function loadEnrichedContent(slug, enrichDir) {
  const dir = enrichDir ?? import_node_path2.default.join("data", "enriched");
  try {
    const raw = await (0, import_promises2.readFile)(import_node_path2.default.join(dir, slug, "content.json"), "utf8");
...
var import_promises2, import_node_path2, DEFAULT_SITE_URL, YMYL_TERMS, TODO_SECTIONS_PER_PAGE, YMYL_DISCLAIMER_TEXT, COMMENT_BLOCKS, ADAPTERS;
var init_generate = __esm({
  "src/core/generate.ts"() {
    ...
    TODO_SECTIONS_PER_PAGE = 4;
    YMYL_DISCLAIMER_TEXT = "This content is for informational purposes only and does not constitute professional advice. Consult a qualified professional before making any decisions based on this information.";
    COMMENT_BLOCKS = {
      nextjs: [
        "// SOPHON GENERATED",
        "// Do not invent statistics, prices, comparisons, or factual claims",
...
var import_promises9, import_node_path10, ADAPTERS2;
var init_diff = __esm({
  "src/core/diff.ts"() {
    "use strict";
...
async function enrich(options) {
  ...
  const toEnrich = [];
  for (const entity of options.entities) {
    const outputPath = import_node_path3.default.join(outputRoot, entity.slug, "content.json");
    if (!options.force) {
      try {
...
async function technical(options) {
  ...
  if (sitemapIndexResult) {
    sitemapWrites.push(
      writeGeneratedFile(import_node_path4.default.join(outputRoot, "sitemap.xml"), sitemapIndexResult.index, { force: options.force })
    );
    for (const sm of sitemapIndexResult.sitemaps) {
      sitemapWrites.push(
        writeGeneratedFile(import_node_path4.default.join(outputRoot, sm.name), sm.content, { force: options.force })
      );
    }
  } else {
    sitemapWrites.push(
      writeGeneratedFile(import_node_path4.default.join(outputRoot, "sitemap.xml"), sitemap, { force: options.force })
    );
  }
  await Promise.all([
    ...sitemapWrites,
...
`, {
    ...
    writeGeneratedFile(
      import_node_path4.default.join(technicalRoot, "breadcrumbs.json"),
...
`,
    ...
    faqSchemas.length > 0 ? writeGeneratedFile(
      import_node_path4.default.join(technicalRoot, "faq-schema.json"),
...
async function walkFiles(root) {
  ...
  async function walk(current) {
    const entries = await (0, import_promises4.readdir)(current, { withFileTypes: true });
    for (const entry of entries) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (matches) {
        ...
        if (!hasType || !hasName) {
          return { valid: false, details: `JSON-LD in ${import_node_path5.default.basename(file)} missing required fields (need: ${requiredFields.join(", ")})` };
        }
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (titleMatch?.[1]) {
        ...
        if (titles.has(title)) {
          duplicates.push(`Duplicate title "${title.slice(0, 40)}..." in ${import_node_path5.default.basename(file)} and ${titles.get(title)}`);
        }
        titles.set(title, import_node_path5.default.basename(file));
      }
      if (descMatch?.[1]) {
        const desc = descMatch[1];
        if (descriptions.has(desc)) {
          duplicates.push(`Duplicate description in ${import_node_path5.default.basename(file)} and ${descriptions.get(desc)}`);
        }
        descriptions.set(desc, import_node_path5.default.basename(file));
      }
    } catch {
    }
  }
...
8. Do you have an ANTHROPIC_API_KEY for AI content enrichment? (${VALID_AI_ANSWERS.join(" / ")})
   > `,
    ...
    await (0, import_promises5.writeFile)(outputPath, formatContext(answers), "utf8");
    console.log(`
...
function findMatchingPage(entity, gscPages, siteUrl) {
  ...
  const endsWith = gscPages.find((p) => {
    const path12 = urlPath(p.page);
    return path12 === `/${slug}` || path12 === `/${slug}/`;
  });
  if (endsWith) return endsWith;
  const contains = gscPages.find((p) => {
    const path12 = urlPath(p.page);
    return path12.includes(`/${slug}`);
  });
...
async function applyEntityFixes(result, entity, outputRoot) {
  ...
  const enrichedPath = import_node_path7.default.join(outputRoot, "data", "enriched", entity.slug, "content.json");
  try {
    assertSafePath(enrichedPath);
    const raw = await (0, import_promises7.readFile)(enrichedPath, "utf8");
...
async function optimize(options) {
  const { site, entities, limit, autoFix, output: output3 } = options;
  ...
  const outputPath = output3 ?? import_node_path8.default.join("data", "optimization-report.json");
  await writeGeneratedFile(
    outputPath,
...
async function blog(options) {
  const outputRoot = options.output ?? import_node_path9.default.join("data", "blog");
  const postsPerEntity = options.postsPerEntity ?? 2;
  const outlines = generateBlogOutlines(options.entities, postsPerEntity);
  await writeGeneratedFile(
    import_node_path9.default.join(outputRoot, "blog-outlines.json"),
...
function validateOutputPath(outputPath) {
  ...
  return import_node_path11.default.resolve(outputPath);
}
function parseCli() {
...
function defaultOutputRoot3(framework) {
  switch (framework) {
    case "nextjs":
      return "app";
    case "astro":
      return import_node_path11.default.join("src", "pages");
    case "nuxt":
      return "pages";
    case "sveltekit":
      return import_node_path11.default.join("src", "routes");
    case "remix":
      return import_node_path11.default.join("app", "routes");
  }
}
async function readJsonIfExists(filePath) {
  try {
    const raw = await (0, import_promises10.readFile)(filePath, "utf8");
    return JSON.parse(raw);
  } catch (error) {
...
async function detectFramework() {
  const packageJson = await readJsonIfExists(import_node_path11.default.join(process.cwd(), "package.json"));
  const dependencies = {
...
async function promptFramework() {
  const rl = (0, import_promises11.createInterface)({ input: import_node_process2.stdin, output: import_node_process2.stdout });
  try {
...
async function readConfig() {
  const config = await readJsonIfExists(import_node_path11.default.join(process.cwd(), "sophon.config.json"));
  return config;
}
...
async function initCommand(values) {
  ...
  const config = {
    framework,
    entitiesPath: import_node_path11.default.join("data", "entities.json"),
    pagesOutput: defaultOutputRoot3(framework),
    technicalOutput: "public",
    enrichOutput: import_node_path11.default.join("data", "enriched")
  };
  await writeGeneratedFile(
    import_node_path11.default.join(process.cwd(), "sophon.config.json"),
...
async function discoverCommand(values) {
  ...
  const outputPath = safeOutput(asString(values["discover-output"]) ?? asString(values.output)) ?? import_node_path11.default.join("data", "entities.json");
...
async function proposeCommand(values) {
  ...
  const outputPath = safeOutput(asString(values["propose-output"]) ?? asString(values.output)) ?? import_node_path11.default.join("data", "proposed-entities.json");
  await writeGeneratedFile(outputPath, `${JSON.stringify(result, null, 2)}
`, {
...
async function generateCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
...
async function technicalCommand(values) {
  ...
  await technical({
    ...
    force: Boolean(values.force),
    maxLinks: Number.parseInt(asString(values["max-links"]) ?? "", 10) || void 0
  });
}
async function enrichCommand(values) {
  ...
  await enrich({
    ...
    concurrency: Number.parseInt(asString(values.concurrency) ?? "", 10) || void 0,
    force: Boolean(values.force),
...
async function runCommand(values) {
  const config = await readConfig();
  const framework = await resolveFramework(asString(values.framework));
  const discoverOutput = safeOutput(asString(values["discover-output"]) ?? asString(values.output)) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const generateOutput = safeOutput(asString(values["generate-output"])) ?? config?.pagesOutput ?? defaultOutputRoot3(framework);
  const technicalOutput = safeOutput(asString(values["technical-output"])) ?? config?.technicalOutput ?? "public";
  const enrichOutput = safeOutput(asString(values["enrich-output"])) ?? config?.enrichOutput ?? import_node_path11.default.join("data", "enriched");
...
async function optimizeCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
  const site = asString(values.site);
  ...
  await optimize({
    ...
    output: safeOutput(asString(values.output)) ?? import_node_path11.default.join("data", "optimization-report.json"),
...
async function scoreCommand(values) {
  ...
  const outputPath = safeOutput(asString(values.output)) ?? import_node_path11.default.join("data", "scores.json");
  await writeGeneratedFile(outputPath, `${JSON.stringify(result, null, 2)}
`, {
    force: Boolean(values.force)
  });
...
async function blogCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
  await blog({
    entities: payload.entities,
    output: safeOutput(asString(values.output)) ?? import_node_path11.default.join("data", "blog"),
    postsPerEntity: Number.parseInt(asString(values["posts-per-entity"]) ?? "", 10) || void 0
  });
}
async function keywordsCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
  const keywordDataPath = asString(values["keyword-data"]);
  const imported = keywordDataPath ? await importKeywordData(keywordDataPath) : void 0;
  const results = analyzeKeywords(payload.entities, imported);
  const outputPath = safeOutput(asString(values.output)) ?? import_node_path11.default.join("data", "keywords.json");
...
async function qualityCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
  const contentDir = safeOutput(asString(values.output)) ?? config?.enrichOutput ?? import_node_path11.default.join("data", "enriched");
  const contentMap = /* @__PURE__ */ new Map();
  for (const entity of payload.entities) {
    try {
      const raw = await (0, import_promises10.readFile)(import_node_path11.default.join(contentDir, entity.slug, "content.json"), "utf8");
      contentMap.set(entity.slug, raw);
    } catch {
    }
  }
  const report = scoreAllContent(payload.entities, contentMap);
  const reportPath = safeOutput(asString(values.output)) ?? import_node_path11.default.join("data", "quality-report.json");
...
async function humanizeCommand(values) {
  ...
  const raw = await (0, import_promises10.readFile)(inputPath, "utf8");
  const result = humanize(raw);
  console.log(result);
}
async function staleCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
...
async function diffCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path11.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
  const framework = await resolveFramework(asString(values.framework));
  const { diffGenerate: diffGenerate2 } = await Promise.resolve().then(() => (init_diff(), diff_exports));
  const result = await diffGenerate2({
    entities: payload.entities,
    framework,
    output: safeOutput(asString(values["generate-output"]) ?? asString(values.output)) ?? config?.pagesOutput,
    site: asString(values.site)
