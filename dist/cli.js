...
function validateOutputPath(outputPath) {
  ...
  return import_node_path10.default.resolve(outputPath);
}
function parseCli() {
...
async function detectFramework() {
  const packageJson = await readJsonIfExists(import_node_path10.default.join(process.cwd(), "package.json"));
  const dependencies = {
...
async function promptFramework() {
  const rl = (0, import_promises9.createInterface)({ input: import_node_process2.stdin, output: import_node_process2.stdout });
  try {
...
async function readConfig() {
  const config = await readJsonIfExists(import_node_path10.default.join(process.cwd(), "sophon.config.json"));
  return config;
}
...
async function initCommand(values) {
  ...
  const config = {
    framework,
    entitiesPath: import_node_path10.default.join("data", "entities.json"),
    pagesOutput: defaultOutputRoot2(framework),
    technicalOutput: "public",
    enrichOutput: import_node_path10.default.join("data", "enriched")
  };
  await writeGeneratedFile(
    import_node_path10.default.join(process.cwd(), "sophon.config.json"),
...
async function discoverCommand(values) {
  ...
  const outputPath = safeOutput(asString(values["discover-output"]) ?? asString(values.output)) ?? import_node_path10.default.join("data", "entities.json");
...
async function proposeCommand(values) {
  ...
  const outputPath = safeOutput(asString(values["propose-output"]) ?? asString(values.output)) ?? import_node_path10.default.join("data", "proposed-entities.json");
  await writeGeneratedFile(outputPath, `${JSON.stringify(result, null, 2)}
`, {
...
async function generateCommand(values) {
  const config = await readConfig();
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path10.default.join("data", "entities.json");
...
async function runCommand(values) {
  const config = await readConfig();
  const framework = await resolveFramework(asString(values.framework));
  const discoverOutput = safeOutput(asString(values["discover-output"]) ?? asString(values.output)) ?? config?.entitiesPath ?? import_node_path10.default.join("data", "entities.json");
  ...
  const enrichOutput = safeOutput(asString(values["enrich-output"])) ?? config?.enrichOutput ?? import_node_path10.default.join("data", "enriched");
...
async function optimizeCommand(values) {
  ...
  await optimize({
    ...
    output: safeOutput(asString(values.output)) ?? import_node_path10.default.join("data", "optimization-report.json"),
...
async function scoreCommand(values) {
  ...
  const outputPath = safeOutput(asString(values.output)) ?? import_node_path10.default.join("data", "scores.json");
  await writeGeneratedFile(outputPath, `${JSON.stringify(result, null, 2)}
`, {
    force: Boolean(values.force)
  });
...
async function blogCommand(values) {
  ...
  await blog({
    entities: payload.entities,
    output: safeOutput(asString(values.output)) ?? import_node_path10.default.join("data", "blog"),
...
async function keywordsCommand(values) {
  ...
  const outputPath = safeOutput(asString(values.output)) ?? import_node_path10.default.join("data", "keywords.json");
...
async function qualityCommand(values) {
  ...
  const entitiesPath = asString(values.entities) ?? config?.entitiesPath ?? import_node_path10.default.join("data", "entities.json");
  const payload = await loadDiscoverResult(entitiesPath);
  const contentDir = safeOutput(asString(values.output)) ?? config?.enrichOutput ?? import_node_path10.default.join("data", "enriched");
  const contentMap = /* @__PURE__ */ new Map();
  for (const entity of payload.entities) {
    try {
      const raw = await (0, import_promises8.readFile)(import_node_path10.default.join(contentDir, entity.slug, "content.json"), "utf8");
      contentMap.set(entity.slug, raw);
    } catch {
    }
  }
  const report = scoreAllContent(payload.entities, contentMap);
  const reportPath = safeOutput(asString(values.output)) ?? import_node_path10.default.join("data", "quality-report.json");
...
async function humanizeCommand(values) {
  ...
  const raw = await (0, import_promises8.readFile)(inputPath, "utf8");
